...
import { dedupeTagsBySlug, tagToSlug, type TagInfo } from './tags'
...
export function getUniqueTagsBySlug(collections: Collections): TagInfo[] {
  return dedupeTagsBySlug(getAllTags(collections))
}

export function getUniqueTagsWithCountBySlug(
  collections: Collections
): Array<TagInfo & { count: number }> {
  const counts = new Map<string, { label: string; count: number }>()
  for (const raw of getAllTags(collections)) {
